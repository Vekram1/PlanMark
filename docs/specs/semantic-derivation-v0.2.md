# PlanMark Semantic Derivation Policy v0.2

## Status

- Policy ID: `semantic_derivation/v0.2`
- Scope: deterministic rules for deriving Semantic IR from Source IR
- Canonical dependency: `docs/specs/ir-v0.2.md`
- Prior version: `docs/specs/semantic-derivation-v0.1.md`
...
Policy v0.2 makes one material derivation change:

- Promoted heading tasks now lift scoped freeform body content into deterministic semantic `sections`.

This means:

- task meaning is no longer limited to title, deps, acceptance, steps, and evidence references
- heading-task rationale and execution guidance inside task scope can participate directly in semantic fingerprints
- tracker sync can detect and project changes to that richer scoped content without relying only on provenance or title changes

This is a semantic derivation change, not just a renderer change, so the policy version is bumped.
...
- Known metadata keys map to typed semantic fields (`horizon`, `deps`, `accept`, `why`, `touches`, and similar policy-approved fields).
...
- Empty leading and trailing blank lines are trimmed; runs of interior blank lines are normalized deterministically.
...
- Semantic output ordering is stable:
  ...
  - section bodies in scoped source order
